...
import { __updateComission, __fetchComission } from '../redux/slices/comissionSlice';

const Comission_Detail = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { comissions, isLoading, error } = useSelector((state) => state.comission);
  ...
  const handleUpdate = async (e) => {
    ...
    setUpdateError(null);
    try {
      await dispatch(__updateComission({ id, title: editedTitle, body: editedBody })).unwrap();
      setIsEditing(false);
    } catch (error) {
  ...
  return (
    <div>
      ...
      <button onClick={() => navigate('/comissionlist')}>Back to List</button>
    </div>
  );
};
